...
Of course, several common matrix operations for genotypes consume even more memory, so more free memory will be needed to accomplish anything.
...
The `BEDMatrix` reader is also written in `Rcpp`, but since its goal is to provide efficient random access to genotypes stored in a file, it sacrifices speed when the goal is to read the entire data into an R matrix.
