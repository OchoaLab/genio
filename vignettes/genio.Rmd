...
## by Iñaki Úcar
required <- c("lfa", "BEDMatrix", "snpStats") # first is not a CRAN package, only suggested since popkin doesn't need it to run... might as well do the same for other dependencies
...
To create annotation tables that look a bit more interesting than the defaults, let us create some slighly more realistic values.
...
Now we similarly create and edit the individiual annotations table.
```{R}
...
file_plink <- 'vignette-random-data'
...
# The same random genotypes (first 10 individuals and loci):
...
```{r}
4 * 1000 * 500000 / 1024 /1024 /1024
```
This is the amount of free memory required just to load the matrix.
Of course, several common matrix operations for genotypes consume even more memory, so more free memory will be needed to acomplish anything.
...
# which can take time for large files
...
# Also note the column and row names.
...
file_plink_copy <- 'vignette-random-data-copy'
...
The `genio` reader is the fastest (as seen below) for several reasons.
...
The `BEDMatrix` reader is also written in `Rcpp`, but since its goal is to provide efficient random access to genotypes stored in a file, it sacrifies speed when the goal is to read the entire data into an R matrix.
...
The `lfa` reader is written in pure R, which is part of the reason it is the slowest in this comparison.
The annotation tables are read with `read.table`, as in `snpStats`.
...
Although `snpStats` does manage memory better than `genio`, is is strictly by a fixed factor of 4.
The reason is that each genotype is encoded in a native R matrix using 4 bytes (when encoded as an integer, as the original matrix and the matrix `genio` read; this is actually platform dependent).
...
Overall, there is a narrow window in data sizes in which a genotype matrix is too large to be encoded as a native R matrix but not too large that it will fit as a `snpStats` object.
That, and the very different interface of `snpStats` makes it not very worthwhile in my opinion, unless there are very specific functions provided by that package that you need and are not implemented elsewhere.
